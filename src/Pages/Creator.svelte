<script>
  ...
  if (
    ...
    window.history.pushState(null, null, "./#creator");
  ...
  onMount(() => {
    ...
    editor.on("text-change", function(delta, oldDelta, source) {
      if (
        source === "user" &&
        oldDelta.ops.length !== 1 &&
        oldDelta.ops[0].insert !== "\n"
      ) {
        changes = true;
      }
      if (editor.getLength() - 2 === delta.ops[0].retain)
        document
          .querySelector("#creator-main")
          .scrollTo(0, document.querySelector("#creator-main").scrollHeight);
    });
    if (db !== null && id !== null) {
      let objectStore = db
  ...
  <div class="body">
    <div class="content" style="max-width: {width_content};">
      ...
      <div class="color-selector">
        ...
        <div id="color" on:click={changeSelect} />
      </div>
      <div id="editor-creator" class="text" />
    </div>
  </div>
</main>
